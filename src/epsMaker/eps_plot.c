  ...
  if (*Nusing==0)
   {
    ...
    if (*usingList==NULL) { sprintf(errtext, "Out of memory."); if (DEBUG) ppl_log(&c->errcontext, errtext); return 1; }
  ...
  else if ((*Nusing==1) && (*NExpect==2)) // Prepend data point number if only one number specified in using statement
   {
    ...
    pplExpr_free(exptmp);
    if ((*usingList)[0]==NULL) { sprintf(errtext, "Out of memory."); if (DEBUG) ppl_log(&c->errcontext, errtext); return 1; }
    *Nusing=2;
   }
