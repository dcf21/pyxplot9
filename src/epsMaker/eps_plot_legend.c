      ...
      if (cptr!=NULL)
       {
        ...
        if (!pd->function) { cptr[k++]=' '; ppl_strEscapify(pd->filename, buffer); inlen=strlen(buffer); BracketLevel=0; status=-1; texify_generic(buffer, &inlen, cptr+k, LSTR_LENGTH-k); k+=strlen(cptr+k); } // Filename of datafile we are plotting
        else
         for (j=0; j<pd->NFunctions; j++) // Print out the list of functions which we are plotting
          {
           ...
           texify_generic(pd->functions[j]->ascii, &inlen, cptr+k, LSTR_LENGTH-k);
           k+=strlen(cptr+k);
          }
        ...
        if (pd->label!=NULL) { sprintf(cptr+k, " label "); k+=strlen(cptr+k); inlen=strlen(pd->label->ascii); BracketLevel=0; status=-1; texify_generic(pd->label->ascii, &inlen, cptr+k, LSTR_LENGTH-k); k+=strlen(cptr+k); } // Print label string
        if (pd->SelectCriterion!=NULL) { strcpy(cptr+k, " select "); k+=strlen(cptr+k); inlen=strlen(pd->SelectCriterion->ascii); BracketLevel=0; status=-1; texify_generic(pd->SelectCriterion->ascii, &inlen, cptr+k, LSTR_LENGTH-k); k+=strlen(cptr+k); } // Print select criterion
          ...
          for (j=0; j<pd->NUsing; j++)
           {
            ...
            texify_generic(pd->UsingList[j]->ascii, &inlen, cptr+k, LSTR_LENGTH-k);
            k+=strlen(cptr+k);
           }
         }
        cptr[k]='\0';
