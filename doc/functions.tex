...
\funcdef{texify($s$)}{takes a string representation of an algebraic expression as its input, e.g. ``\texttt{(x/3)**2}'', and returns a \LaTeX\ representation of it.}
\funcdef{texifyText($s$)}{returns a string of \LaTeX\ text corresponding to the supplied text string, with any reserved characters escaped.}
...
The \texttt{time} module contains functions for handling objects of type \texttt{date}:

\funcdef{time.fromCalendar($year,month,day,hour,min,sec$)}{creates a date object from the specified calendar date. See also the \texttt{set calendar} command to change the current calendar.}
...
\protdef{types.matrix(...)}{{\bf none} -- returns a one-by-one zero matrix. \item {\bf a pair of numbers} -- returns a zero matrix of the specified dimensions. \item {\bf a matrix} -- returns a copy of the supplied matrix. \item {\bf a vector} -- converts the supplied vector into a matrix object. \item {\bf a list of lists or vectors} -- converts the supplied data structure into a matrix, providing all the elements are numerical and have matching units.}
...
\protdef{types.number(...)}{{\bf none} -- returns zero. \item {\bf a number} -- returns that number. \item {\bf a boolean} -- returns zero or one. \item {\bf a date} -- returns a numerical Unix time.}
