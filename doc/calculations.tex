...
provides.  Most of the examples in this chapter act on single numerical values,
displaying the results using the {\tt print} command, demonstrating how Pyxplot
may be used as a desktop calculator. The next chapter will extend this to use of
...
Time-series data need to be handled carefully, since it is not straightforward
to convert a series of calendar dates into elapsed times between the \datapoint
s. The problems are especially difficult for series that span many weeks, since
months have non-uniform lengths and some years have an extra leap day. But even
shorter time series are complicated by hours and minutes being non-decimal
fractions of days.
...
2\,{\footnotesize BC}, and so forth. The \texttt{set calendar} command may be used
...
Also, the function {\tt time.now()}\indfun{time.now()}, which takes no
