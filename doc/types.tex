...
\noindent The sections below list the methods of each data type.
...
\methdef{str(...)}{converts a date object to a string with an optional format string supplied as an argument (see the {\tt time.string()} function).}
...
\methdef{sortOnElement($n$)}{sorts a list of lists on the $n$th element of each sublist.}
